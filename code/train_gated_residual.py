...
CONFIDENCE_GATE_THRESHOLD = 0.08  # same as hybrid controller's threshold
...
def main() -> None:
    ...
    rng = np.random.default_rng(92)  # different seed from fixed residual
    ...
    (OUTPUT_DIR / "gated_residual_mlp_training.json").write_text(json.dumps(payload, indent=2))
    np.save(OUTPUT_DIR / "gated_residual_mlp_theta.npy", best_theta)
    print("Gated residual MLP training finished.")
